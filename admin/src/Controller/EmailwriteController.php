...
class EmailwriteController extends DisplayController
{
	...
	function send () {
		...
		$postN  = $this->input->post->get('jform', array(), 'array');
		$post['article_id']	= isset($postN['article_id']) ? (int)$postN['article_id'] : '';
		...
		.'&ext=virtuemart&type='.$post['type'].'&order_id='.(int)$post['order_id'].'&delivery_id='.(int)$post['delivery_id'];
			...
			$app->setUserState( $context.'article_id', $post['article_id']);
		...
		$send = SendHelper::send($post, $warning, $error, $redirect );
