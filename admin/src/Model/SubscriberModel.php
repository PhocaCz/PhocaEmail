...
class SubscriberModel extends AdminModel
{
	...
	public function getForm($data = array(), $loadData = true) {
		$form 	= $this->loadForm('com_phocaemail.subscriber', 'subscriber', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
		return $form;
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_phocaemail.edit.subscriber.data', array());
	...
	/*	if ($data['active'] == 1 && !isset($data['date_register'])) {
		...
		try
		{
			...
			SendnewsletteremailHelper::storeLists($data['id'], $data['mailinglist'], '#__phocaemail_subscriber_lists', 'id_subscriber');
