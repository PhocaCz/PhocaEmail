...
/*
	...
	HTMLHelper::_('behavior.modal', 'a.modal_jform_request_id');

	$idA 	= 'phSelectArticle';
	$id		= 'jform_request_id';
	$link 	= 'index.php?option=com_content&amp;view=articles&amp;layout=modal&amp;tmpl=component&amp;function=jSelectArticle_'.$id.'&amp;'.Session::getFormToken().'=1';
	...
	$html[] = '<span class="input-append"><input type="text" id="article_name" name="article_name"'
			. ' value="' .  $this->re['article_name'] . '" class="input-medium"  />';
	$html[] = '<a href="#'.$idA.'" role="button" class="btn " data-toggle="modal" title="' . Text::_('COM_PHOCAEMAIL_SELECT_ARTICLE') . '">'
			. '<span class="icon-list icon-white"></span> '
			. Text::_('COM_PHOCAEMAIL_SELECT_ARTICLE') . '</a></span>';
	$html[] = '</div>'. "\n";
	$html[] = '<input required="required" aria-required="true" type="hidden" id="article_id" name="article_id" value="'.$this->re['article_id'].'" />';
	...
	$html[] = HTMLHelper::_(
		...
		array(
			'url'    => $link,
			'title'  => Text::_('COM_PHOCAEMAIL_SELECT_ARTICLE'),
			'width'  => '780px',
			'height' => '580px',
			'modalWidth' => '50',
			'bodyHeight' => '70',
			'footer' => '<button type="button" class="btn" data-dismiss="modal" aria-hidden="true">'
				. Text::_('COM_PHOCAEMAIL_CLOSE') . '</button>'
		)
	);

?><tr>
	<td class="ph-label"><label data-original-title="<?php echo Text::_('COM_PHOCAEMAIL_SELECT_ARTICLE'); ?>" id="jform_request_id-lbl" for="jform_request_id" class="hasTooltip required" title=""><?php echo Text::_('COM_PHOCAEMAIL_SELECT_ARTICLE'); ?>:</label>
	</td>
...
<tr>
	<td class="ph-label" valign="top"><label for="title"><?php echo Text::_('COM_PHOCAEMAIL_FIELD_MESSAGE');  ?>:</label></td><td>
...
if ($this->re['ext'] == 'virtuemart') {
	echo '<input type="hidden" name="order_id" value="'.$this->re['order_id'].'" />'. "\n";
	echo '<input type="hidden" name="delivery_id" value="'.$this->re['delivery_id'].'" />' . "\n";
	echo '<input type="hidden" name="type" value="'.$this->re['type'].'" />' . "\n";
	echo '<input type="hidden" name="ext" value="'.$this->re['ext'].'" />' . "\n";
}
/*
...
<div id="ph-op-bg"></div>
<div id="ph-op-bg-msg"><?php echo HTMLHelper::_('image', 'media/com_phocaemail/images/administrator/icon-sending.gif', '' ) . ' &nbsp; &nbsp; '. Text::_('COM_PHOCAEMAIL_SENDING_MESSAGE'); ?></div>
